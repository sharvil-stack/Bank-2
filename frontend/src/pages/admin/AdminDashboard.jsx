...
const AdminDashboard = () => {
    ...
    const avgBalance     = totalAccounts > 0 ? (totalBalance / totalAccounts).toFixed(2) : 0
    ...
    return (
        <div>
            ...
            <div className="admin-stats-grid">
                ...
                <StatCard label="Total Balance" value={`₹${Number(totalBalance).toLocaleString('en-IN')}`} accent="positive" />
                <StatCard label="Avg Balance / Account" value={`₹${Number(avgBalance).toLocaleString('en-IN')}`} />
            </div>
                        ...
                        {topAccounts.length === 0
                            ...
                            : (
                                <table className="admin-table">
                                    <thead><tr><th>Account No.</th><th>Balance</th><th>Status</th></tr></thead>
                                    <tbody>
                                        {topAccounts.map(a => (
                                            <tr key={a.accountNumber}>
                                                <td style={{ fontFamily: 'var(--mono)', fontSize: 13 }}>{a.accountNumber}</td>
                                                <td style={{ fontWeight: 600, color: 'var(--accent)' }}>₹{Number(a.balance).toLocaleString('en-IN')}</td>
                                                <td><span className={`admin-badge ${a.status?.toLowerCase()}`}>{a.status}</span></td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            ...
                            : (
                                <table className="admin-table">
                                    ...
                                    <tbody>
                                        {recentUsers.map(u => (
                                            <tr key={u.id}>
                                                <td style={{ fontWeight: 600, color: 'var(--text-h)' }}>{u.firstName} {u.lastName}</td>
                                                <td style={{ fontSize: 13, color: 'var(--text-muted)' }}>{u.email}</td>
                                                <td><span className={`admin-badge role-${u.role?.toLowerCase()}`}>{u.role}</span></td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
